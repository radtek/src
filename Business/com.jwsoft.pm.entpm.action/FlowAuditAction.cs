namespace com.jwsoft.pm.entpm.action
{
    ...
    public class FlowAuditAction
    {
        ...
        {
            int num = 0;
            string str1 = string.Concat(new object[] { " select ID from WF_Instance_Main  where InstanceCode='", instanceCode, "' and businessclass='", BusinessClass, "' and businesscode='", BusinessCode, "' ORDER BY StartTime DESC " });
            DataTable table = publicDbOpClass.DataTableQuary(str1);
            bool flag = false;
            ...
            string str2 = "select Sing  from WF_Instance where ID='" + str + "' and TheOrder=(select MAX(TheOrder) from WF_Instance where ID='" + str + "')";
            DataTable table2 = publicDbOpClass.DataTableQuary(str2);
            if (table2.Rows.Count > 0)
            {
                bool flag2 = false;
