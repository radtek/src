namespace com.jwsoft.pm.entpm.action
{
    ...
    public class PTDUTYAction
    {
        ...
        private string GenerateCondition(string employeeCode, string employeeName, string department, string post, string educationalBackground, string classID, string postAndRank, string positionLevel, string ddlWXZT)
        {
            StringBuilder builder = new StringBuilder();
            ...
            if (!string.IsNullOrEmpty(ddlWXZT))
            {
                builder.AppendFormat("AND y.WXZT = {0} \n", ddlWXZT);
            }
        ...
        public int GetEmployees_Count(string employeeCode, string employeeName, string department, string post, string educationalBackground, string classID, string postAndRank, string positionLevel, string ddlWXZT)
        {
            string str = this.GenerateCondition(employeeCode, employeeName, department, post, educationalBackground, classID, postAndRank, positionLevel, ddlWXZT);
            StringBuilder builder = new StringBuilder();
        ...
        public DataTable GetEmployeesData(string employeeCode, string employeeName, string department, string post, string educationalBackground, string classID, string postAndRank, string positionLevel, int pageIndex, int pageSize,string ddlWXZT)
        {
            string str = this.GenerateCondition(employeeCode, employeeName, department, post, educationalBackground, classID, postAndRank, positionLevel, ddlWXZT);
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat(" SELECT TOP({0}) * FROM ( \n", pageSize);
            builder.Append("\t    SELECT Row_Number()OVER(ORDER BY PT_d_bm.i_xh, y.I_DUTYID) AS Num , \n");
            builder.Append("\t\ty.v_yhdm,y.WXID,case when y.WXZT=1 then '已激活' when y.WXZT=2 then '已禁用' when y.WXZT=4 then '未激活' else '' end WXZT ,ISNULL(y.UserCode,'') UserCode,V_Dlid,v_xm,y.i_bmdm, \n");
            builder.Append("     v_bmqc,y.I_DUTYID,DutyName RoleTypeName, \n");
            builder.Append("\t\tOtherDepts, OtherDutyIDs, y.i_xh, PT_d_bm.i_xh AS Bmxh, \n");
            builder.Append("\t\ty.c_sfyx, MobilePhoneCode, RelationCorp, EnterCorpDate, Age, State, \n");
            builder.Append("\t\t(case State when 0 then '试用' when 1 then '在职' when 2 then '离职' end ) as StateName, \n");
