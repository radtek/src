namespace cn.justwin.stockBLL
{
    ...
    public class WorkFlow
    {
        ...
        public static void DealOutTimeAudit()
        {
            ...
            if (table.Rows.Count > 0)
            {
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    DataTable table2 = SqlHelper.ExecuteQuery(CommandType.Text, builder.ToString(), new SqlParameter[0]);
                    if (table2.Rows.Count == 1)
                    {
                        ...
                        DataTable table3 = SqlHelper.ExecuteQuery(CommandType.Text, builder2.ToString(), new SqlParameter[0]);
        ...
        public static string GetWFState(string businesscode, string instancecode)
        {
            ...
            try
            {
                str = SqlHelper.ExecuteQuery(1, " select  " + table.Rows[0]["StateField"].ToString() + " from  " + table.Rows[0]["LinkTable"].ToString() + "  where " + table.Rows[0]["PrimaryField"].ToString() + " ='" + instancecode + "' ", new SqlParameter[0]).Rows[0][0].ToString();
            }
            catch
            {
            }
