			...
			else
			{
				if (!string.IsNullOrEmpty(this.pcode))
				{
                    ...
                    for (int i = 0; i < purchaseStockByPscode.Rows.Count; i++)
                    {
                        ...
                        try
                        {
                            DataTable resByBud = this.materialStock.GetResByBud("", "", "", "", this.hfldProject.Value, 1, 9999999, 1, isWBSRelevance, "");
    ...
    public void AddWX()
    {
        using (SqlConnection sqlConnection = new SqlConnection(SqlHelper.ConnectionString))
        {
            ...
            try
            {
                ...
                sqlTransaction.Commit();
                System.Text.StringBuilder stringBuilder = new System.Text.StringBuilder();
                stringBuilder.Append("alert('添加成功');").Append(System.Environment.NewLine);
            ...
            catch (System.Exception ex)
            {
                base.Trace.Write(ex.Message);
                sqlTransaction.Rollback();
                base.RegisterScript("alert('添加失败');");
            }
        }
    }
    private void Update()
    ...
    private void UpdateWX()
    {
        using (SqlConnection sqlConnection = new SqlConnection(SqlHelper.ConnectionString))
        {
            ...
            try
            {
                ...
                sqlTransaction.Commit();
                System.Text.StringBuilder stringBuilder = new System.Text.StringBuilder();
                stringBuilder.Append("alert('修改成功');").Append(System.Environment.NewLine);
                stringBuilder.Append("parent.location.reload();");
                //stringBuilder.Append("winclose('PurchaseEdit', 'Purchase.aspx?PrjGuid=" + this.hfldProject.Value + "', true)");
                base.RegisterScript(stringBuilder.ToString());
            }
		...
		if (this.ViewState[StockManage_Purchase_PurchaseEdit.resourceDataSourceName] is DataTable)
		{
			...
			for (int i = 0; i < dataTable.Rows.Count; i++)
			{
                ...
                */
                ...
                if (gridViewRow.FindControl("txtTotal") is TextBox)
                {
                    TextBox textBox3 = gridViewRow.FindControl("txtTotal") as TextBox;
                    dataRow["Total"] = textBox3.Text.Trim();
                }

                dataRow["sprice"] = System.Convert.ToDecimal(dataRow["Total"]) / System.Convert.ToDecimal(dataRow["number"]);
                String abc = dataRow["sprice"].ToString();
                String ab2 = dataRow["Total"].ToString();
                String ab3 = dataRow["number"].ToString();
