...
(function($){
    ...
    $.iviewer = function(e,o)
    {
        ...
        */
        ...
        touchstart(function (ev, e) {
            //alert(11);
            var touchPoints=ev.originalEvent.touches.length;
            var ex = ev.originalEvent.touches;
            //alert(touchPoints);
            //判断触点个数
            if(touchPoints==1){
                if(canSign==1){
                    return me.click(ex[0]);
                }else{
                    return me.drag_start(ex[0]);
                }
            }else if(touchPoints>=2){
                startMove=ev.originalEvent.touches;
                return false;
            }
        }).touchmove(function(ev,e){
            var touchPoints=ev.originalEvent.touches.length;
            //判断触点个数
            if(touchPoints==1){
                var ex = ev.originalEvent.touches;
                ...
                var now=ev.originalEvent.touches;
                var scale=getDistance(now[0],now[1])/getDistance(startMove[0],startMove[1]);
                var distance = getDistance(now[0],now[1])-getDistance(startMove[0],startMove[1]);
                var zoom;
                if((distance*distance)>=144){
                    if(scale<1){
                        zoom=-1;
                    }else{
                        zoom=1;
                    }
                    me.touchmovezoom_by(zoom);
                }
                startMove=now;
                return false;
            }
        }).touchend(function (ev, e) {
            //alert(12);
            var touchPoints=ev.originalEvent.touches.length;
            //判断触点个数
            if(touchPoints==1){
                var ex = ev.originalEvent.touches;
                return me.drag_end(ex[0]);
            }else if(touchPoints>=2){
                return false;
            }
        });
    ...
    $iv.fn.extend({
        ...
        **/
        ...
        set_touchmovezoom: function(new_zoom)
        {
            ...
            this.update_status();
			//图片缩放后执行onZoom事件
			if(this.settings.onTouchMove && this.settings.onTouchMove.call(this, new_zoom) == false)
            {
                return;
            }
        },
        ...
        **/
        ...
        touchmovezoom_by: function(delta)
        {
            var closest_rate = this.find_closest_zoom_rate(this.current_zoom);
            ...
            this.set_touchmovezoom(next_zoom);
        },
