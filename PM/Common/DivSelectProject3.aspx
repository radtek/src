<%@ Page Language="C#" AutoEventWireup="true" CodeFile="DivSelectProject3.aspx.cs" Inherits="Common_DivSelectProject3" %>
...
<head runat="server"><title></title>
	...
	<script language="JavaScript" type="text/javascript">
		$(document).ready(function () {
			var jwTable = new JustWinTable('gvwPrj');
		});
		...
		function okEvent() {
			...
			$('#gvwPrj :checked').each(function () {
				var tr = jw.getParentUntil(this, 'TR');
				if (tr.id != '') {
					idArr.push(tr.id);
					nameArr.push($(tr).attr('name'));
				}
			});

			var idJson = jw.array1dToJson(idArr);
			var nameJson = jw.array1dToJson(nameArr);

			if (idJson != '' && nameJson != '') {
				if (typeof top.ui.callback == 'function') {
					top.ui.callback({ id: idJson, name: nameJson });
				}

				top.ui.closeWin();
			}

			//			$('#hfldPrjId').val(idJson);
			//			$('#hfldPrjName').val(nameJson);

			//			alert(nameJson);

			//			if ($('#hfldPrjId').val() == '') return false;
			//			if ($('#hfldPrjName').val() == '') return false;
...
<body>
	...
	<div style="text-align: right">
		<input id="btnSave" type="button" value="确 定" onclick="okEvent();" />
