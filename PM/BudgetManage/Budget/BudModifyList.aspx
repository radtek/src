<%@ Page Language="C#" AutoEventWireup="true" CodeFile="BudModifyList.aspx.cs" Inherits="BudgetManage_Budget_BudModifyList" %>
...
<head runat="server"><title></title><link type="text/css" rel="Stylesheet" href="../../Script/jquery.tooltip/jquery.tooltip.css" />
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var jwBudget = new JustWinTable('gvBudget');
        ...
        function openNewTab(action) {
            var title = '新增变更信息';
            //			parent.desktop.EditModify = window;
            top.ui._EditModify = window;
            var urlStr = '/BudgetManage/Budget/EditModifyTest.aspx?' + new Date().getTime() + '&action=' + action + '&prjId=' + $('#hfldPrjId').val() + '&year=' + $('#hfldYear').val();
            if (action == 'Edit') {
        ...
        //查看页面
        function Query() {
            var url = '/BudgetManage/Budget/BudModifyView.aspx?' + new Date().getTime() + '&ic=' + $('#hfldModifyId').val();
            viewopen(url, 820, 500);
        }
        //标签页查看
        function QueryTab(modifyTaskId) {
            //			parent.desktop.BudModifyView = window;
            var url = '/BudgetManage/Budget/BudModifyView.aspx?' + new Date().getTime() + '&ic=' + modifyTaskId;
            toolbox_oncommand(url, "预算变更查看");
        }
...
<body>
    ...
    <div id="divContent2" class="divContent2" style="height: 100%; padding: 0px;">
                                ...
                                <table border="0" class="tab">
                                    <tr id="tr_Buttons">
                                        <td class="divFooter" style="text-align: left" id='td_Buttons'>
                                            ...
                                            <MyUserControl:epc_usercontrol1_wf_ascx ID="WF1" BusiCode="132" BusiClass="001" runat="server" />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="vertical-align: top; height: 100%;">
                                            <div id="divBudget" class="pagediv" style="">
                                                <asp:GridView ID="gvBudget" AutoGenerateColumns="false" CssClass="gvdata" OnRowDataBound="gvBudget_RowDataBound" DataKeyNames="ModifyId" runat="server">
<EmptyDataTemplate>
                                                        <table id="emptyBudget" class="tab" width="100%">
                                                            <tr class="header">
                                                                <td style="width: 20px">
