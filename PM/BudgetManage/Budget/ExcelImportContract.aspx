<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ExcelImportContract.aspx.cs" Inherits="BudgetManage_Budget_ExcelImportContract" %>
        ...
        $(document).ready(function () {
            ...
            $('#btmImport').click(function () {
                ...
                if (columns.length <= $('#gvwWBSData select').length) {
                    ...
                    return false;
                } else if (columns.indexOf('SerialNo') == -1) {
                    //top.ui.alert('"WBS信息：序号"必需选择！');
                    //return false;
                } else if (columns.indexOf('TaskCode') == -1) {
                    top.ui.alert('"WBS信息：任务编码"必需选择！');
                    return false;
                } else if (columns.indexOf('TaskName') == -1) {
                    top.ui.alert('"WBS信息：任务名称"必需选择！');
                    return false;
                    ...
                    if ($('#hfldValidRes').val() != "0") {
                        ...
                        if (columnsRe.length <= $('#gvwResData select').length) {
                            ...
                            return false;
                        } else if (columnsRe.indexOf('SerialNo') == -1) {
    ...
    <table cellpadding="0px" cellspacing="0px" style="width: 100%; vertical-align: top;">
        <tr>
            <td class="divFooter" style="text-align: left;">
                ...
                <asp:Button ID="btnDel" Text="删除" OnClientClick="return confirm('您确定要删除吗？')" disabled="disabled" Style="cursor: pointer;" OnClick="btnDel_Click" runat="server" />
            </td>
        </tr>
...
<ItemTemplate>
                        ...
                        <div style="float: left;">
                            <webdiyer:AspNetPager ID="AspNetPagerRes" Width="100%" ShowPageIndexBox="Always" PageIndexBoxType="DropDownList" TextBeforePageIndexBox="转到: " FirstPageText="首页" LastPageText="末页" PrevPageText="上一页" NextPageText="下一页" HorizontalAlign="Right" EnableTheming="true" OnPageChanged="AspNetPagerRes_PageChanged" runat="server">
                            </webdiyer:AspNetPager>
                        </div>
                    </div>
                </div>
