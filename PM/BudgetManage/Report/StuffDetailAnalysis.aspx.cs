...
using Wuqi.Webdiyer;
public partial class BudgetManage_Report_StuffDetailAnalysis : NBasePage, IRequiresSessionState
{
	...
	public void BindGv()
	{
		...
		string resourceCode = this.txtResourceCode.Text.Trim();
		string resourceName = this.txtResourceName.Text.Trim();
		this.AspNetPager1.RecordCount = ConstructReport.GetCBSTaskCount(taskCode, taskName, resourceCode, resourceName, this.prjId, "StuffCBSCode");
		DataTable cBSTaskAnalysis = ConstructReport.GetCBSTaskAnalysis(taskCode, taskName, resourceCode, resourceName, this.prjId, "StuffCBSCode", this.AspNetPager1.PageSize, this.AspNetPager1.CurrentPageIndex);
		this.gvStuffCost.DataSource = cBSTaskAnalysis;
		this.gvStuffCost.DataBind();
	}
	protected void gvStuffCost_RowDataBound(object sender, GridViewRowEventArgs e)
	{
	...
	protected void btnExcel_Click(object sender, System.EventArgs e)
	{
		...
		string resourceCode = this.txtResourceCode.Text.Trim();
		string resourceName = this.txtResourceName.Text.Trim();
		dataTable = ConstructReport.GetCBSTaskAnalysis(taskCode, taskName, resourceCode, resourceName, this.prjId, "StuffCBSCode", 0, 0);
		if (dataTable.Rows.Count > 0)
		{
			...
			dataRow["OutPrice"] = dataTable.Compute("sum(OutPrice)", "1>0");
			dataTable.Rows.Add(dataRow);
		}
		dataTable = this.GetTitleByTable(dataTable);
		ExcelHelper.ExportExcel(dataTable, "材料明细表", "材料明细表", "材料明细表.xls", null, null, 2, base.Request.Browser.Browser);
	}
	public DataTable GetTitleByTable(DataTable dt)
	{
		...
		if (dt.Columns["OutPrice"] != null)
		{
			dt.Columns["OutPrice"].ColumnName = "出库金额";
		}
		dt.AcceptChanges();
		return dt;
	}
