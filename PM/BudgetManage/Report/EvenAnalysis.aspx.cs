...
public partial class BudgetManage_Report_EvenAnalysis : NBasePage, IRequiresSessionState
{
	...
	protected void gvCost_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.Header)
		{
			...
			cells[8].RowSpan = 2;
			cells[8].Text = "盈亏";
			cells[8].Attributes.Add("title", "盈亏 = 合同金额 &ndash; 实际成本");
			cells[8].CssClass = "tooltip";
			cells.Add(new TableHeaderCell());
			cells[9].RowSpan = 2;
			cells[9].Attributes.Add("title", "盈亏率 = 盈亏 &divide; 合同金额");
			cells[9].CssClass = "tooltip";
			cells[9].Text = "盈亏率</th></tr><tr class='header'>";
	...
	public DataTable GetTitleByTable(DataTable dt)
	{
		...
		{
			dt.Columns["ContractBud"].ColumnName = "合同金额";
		}
        if (dt.Columns["ysk"] != null)
        {
            dt.Columns["ysk"].ColumnName = "收歀累计";
        }
        if (dt.Columns["dsk"] != null)
        {
            dt.Columns["dsk"].ColumnName = "待收款";
        }
