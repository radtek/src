...
using System.Web.UI.WebControls;
	public partial class PQueryBuild : PageBase, IRequiresSessionState
	{
			...
			if (!this.Page.IsPostBack)
			{
				...
				for (int i = 0; i < this.Options.Rows.Count; i++)
				{
					queryItemCollection.Add(new QueryItem
					{
						...
						ItemSign = this.Options.Rows[i]["TermSign"].ToString(),
						ItemValue = "",
		...
		private void dgdQuery_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			case ListItemType.AlternatingItem:
			{
				...
				int dataType = queryItem.DataType;
				e.Item.Attributes["onclick"] = "doClick(this,'" + this.dgdQuery.ClientID + "');";
			...
			for (int i = 0; i < this.ItemList.Count; i++)
			{
				...
				else
				{
					...
					case 2:
						...
						break;
					case 3:
						if (!VerifyHelper.IsDouble(queryItem.ItemValue))
						{
							text = text + queryItem.ItemName + "类型不正确！\\n";
							flag = false;
						}
