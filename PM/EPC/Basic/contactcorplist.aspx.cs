...
using System.Web.UI.WebControls;
	public partial class ContactCorpList : PageBase, IRequiresSessionState
	{
		...
		private void DgrdList_Bind()
		{
			...
			{
				if (!string.IsNullOrEmpty(this.isAudit))
				{
					stringBuilder.Append(" and CorpTypeID=" + this.CorpTypeID + " and IsValid=0");
				}
				else
				{
			...
			if (this.DgrdList.Items.Count <= 0)
			{
		...
		private void Page_CustomInit()
		{
			...
			this.btnpzdel.Attributes["onclick"] = "javascript:if(!confirm('执行此操作可能会造成其它模块中引用此单位的数据异常显示，您确定要删除吗？')){return false;}";
		}
		...
		{
			this.Cont.ActiveDeptList(int.Parse(this.HdnCorpID.Value));
			this.prebind();
		}
		protected DataTable AllInfo()
		{
			base.Request["cti"].ToString();
			...
			text += "taxcard as 税号,bankaccounts as 银行账号,accountbank as 开户银行,convert(nvarchar(80),corpkind)as 单位性质,Convert(nvarchar(80),worktype) as 经营类别,client  as 委托人,underlayability as 垫资能力,capital as 固定资产 into #NewContactcorp from xpm_basic_contactcorp where ";
			if (!string.IsNullOrEmpty(this.isAudit))
			{
				text += " isvalid=0";
			}
		...
		protected void btnExp_Click1(object sender, EventArgs e)
		{
			...
			excelExporter.Export(this.AllInfo(), "单位信息.xls", base.Request.Browser.Browser);
		}
