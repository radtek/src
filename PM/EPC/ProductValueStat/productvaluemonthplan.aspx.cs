...
using System.Web.UI.WebControls;
	public partial class ProductValueMonthPlan : PageBase, IRequiresSessionState
	{
		...
		private void Init_theSchedule()
		{
			...
			this._DtSchedule.Columns.Add(new DataColumn("PlanOverQuantity", typeof(string)));
			this._DtSchedule.Columns.Add(new DataColumn("PlanAccumulativeQuantity", typeof(string)));
			this._DtSchedule.Columns.Add(new DataColumn("SynthPrice", typeof(string)));
			this._DtSchedule.Columns.Add(new DataColumn("PlanProductValue", typeof(string)));
			ArrayList arrayList = new ArrayList();
			arrayList = this.ProductValueAct.ProductValueMonthStatWeave(this.ProjectCode, this.StartDate, "P");
			for (int i = 0; i < arrayList.Count; i++)
			{
				...
				dataRow["PlanOverQuantity"] = monthProductValueDetail.PlanOverQuantity.ToString();
				dataRow["PlanAccumulativeQuantity"] = monthProductValueDetail.PlanAccumulativeQuantity.ToString();
				dataRow["SynthPrice"] = monthProductValueDetail.SynthPrice.ToString();
				dataRow["PlanProductValue"] = monthProductValueDetail.PlanProductValue.ToString();
		...
		private void tblScheduleView_Draw(DataTable dt)
		{
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				if ((dataRow["WorkLayer"].ToString() == "3" && dataRow["ChildNum"].ToString() == "false") || dataRow["WorkLayer"].ToString() == "4")
				{
					...
					textBox.ID = "txtPlanOverQuantity" + dataRow["TaskCode"].ToString();
					textBox.Text = dataRow["PlanOverQuantity"].ToString();
				...
				if ((dataRow["WorkLayer"].ToString() == "3" && dataRow["ChildNum"].ToString() == "false") || dataRow["WorkLayer"].ToString() == "4")
				{
					tableCell.Text = this.ProductValueAct.AccumulativeSum(this.ProjectCode, dataRow["TaskCode"].ToString(), "P");
				}
				else
				{
					tableCell.Text = "";
				...
				tableRow.Cells.Add(tableCell);
				tableCell = new TableCell();
				if ((dataRow["WorkLayer"].ToString() == "3" && dataRow["ChildNum"].ToString() == "false") || dataRow["WorkLayer"].ToString() == "4false")
				{
					tableCell.Text = Convert.ToDecimal(dataRow["PlanProductValue"].ToString()).ToString();
				}
				else
				{
					tableCell.Text = "";
		...
		private ArrayList GetTable_Data()
		{
			...
			for (int i = 1; i < this.tblScheduleView.Rows.Count; i++)
			{
				...
				monthProductValueDetail.StatDate = this.StartDate;
				monthProductValueDetail.Unit = this.tblScheduleView.Rows[i].Cells[1].Text;
				if (this.tblScheduleView.Rows[i].Cells[7].Text == "3false" || this.tblScheduleView.Rows[i].Cells[7].Text == "4false")
				{
					monthProductValueDetail.Quantity = Convert.ToDecimal(this.tblScheduleView.Rows[i].Cells[2].Text);
					monthProductValueDetail.PlanOverQuantity = Convert.ToDecimal(((TextBox)this.tblScheduleView.Rows[i].Cells[3].Controls[0]).Text);
					monthProductValueDetail.PlanAccumulativeQuantity = Convert.ToDecimal(this.tblScheduleView.Rows[i].Cells[4].Text);
					monthProductValueDetail.SynthPrice = Convert.ToDecimal(this.tblScheduleView.Rows[i].Cells[5].Text);
					monthProductValueDetail.PlanProductValue = Convert.ToDecimal(((TextBox)this.tblScheduleView.Rows[i].Cells[3].Controls[0]).Text) * Convert.ToDecimal(this.tblScheduleView.Rows[i].Cells[5].Text);
				}
				else
				{
					monthProductValueDetail.Quantity = 0m;
					monthProductValueDetail.PlanOverQuantity = 0m;
					monthProductValueDetail.PlanAccumulativeQuantity = 0m;
					monthProductValueDetail.SynthPrice = 0m;
					monthProductValueDetail.PlanProductValue = 0m;
				}
		...
		protected void BtnSave_Click(object sender, EventArgs e)
		{
			...
			if (this.ProductValueAct.AddProductValueMonthPlan(table_Data, this.ProjectCode, this.StartDate) == 1)
			{
				this.RegisterStartupScript("", string.Concat(new object[]
				{
					"<script>alert('保存成功！');location.href = 'ProductValuePlanFrm.aspx?t=",
					this.IsExamine,
					"&pc=",
					this.ProjectCode,
					"'</script>"
		...
		{
			if (this.ProductValueAct.ProductValueExamine(this.ProjectCode, this.StartDate))
			{
				this.BtnExamine.Enabled = false;
				this.RegisterStartupScript("", "<script>alert('审核成功！');</script>");
				return;
			}
			this.RegisterStartupScript("", "<script>alert('审核失败！');</script>");
		}
	}
