...
using Wuqi.Webdiyer;
public partial class ContractManage_ContractReport_IncometContractContrast : NBasePage, IRequiresSessionState
{
	...
	public DataTable GetTable(string strWhere)
	{
		...
		this.contractCount = this.incometContractBll.GetReportTb(this.txtConType.Text.Trim(), this.txtPrjName.Value, this.txtContractCode.Text.Trim(), this.txtContractName.Text.Trim(), this.txtStartTime.Text.Trim(), this.txtEndTime.Text.Trim(), this.txtParty.Value.Trim(), "Contract", strWhere, this.dropPrjKindClass.SelectedValue);
		this.ViewState["contract"] = this.contractCount;
		this.AspNetPager1.RecordCount = this.contractCount.Rows.Count;
		return this.incometContractBll.GetReportTb(this.txtConType.Text.Trim(), this.txtPrjName.Value, this.txtContractCode.Text.Trim(), this.txtContractName.Text.Trim(), this.txtStartTime.Text.Trim(), this.txtEndTime.Text.Trim(), this.txtParty.Value.Trim(), "Contract", strWhere, this.dropPrjKindClass.SelectedValue, this.AspNetPager1.CurrentPageIndex, this.AspNetPager1.PageSize);
	}
	public void BindGv(DataTable storageDataSource)
	{
		...
		{
			this.contractCount.Compute("sum(ContractPrice)", "").ToString(),
			this.contractCount.Compute("sum(EndPrice)", "").ToString(),
			this.contractCount.Compute("sum(ModifyPrice)", "").ToString(),
			this.contractCount.Compute("sum(BalancePrice)", "").ToString(),
			this.contractCount.Compute("sum(PaymentPrice)", "").ToString(),
			this.contractCount.Compute("sum(InvoicePrice)", "").ToString()
		};
		int[] index = new int[]
		{
			3,
	...
	protected void btnexecl_Click(object sender, System.EventArgs e)
	{
		...
		new Common2().ExportToExecelAll(this.GetTitleByTable(dt), "收入合同明细报表.xls", base.Request.Browser.Browser);
	}
	public DataTable GetTitleByTable(DataTable dt)
	{
		dt = WebUtil.UpdateDataTable(dt, "Contrast", "Party");
		if (dt.Rows.Count > 0)
		{
			string[] array = new string[]
			{
				dt.Compute("sum(ContractPrice)", "").ToString(),
				dt.Compute("sum(EndPrice)", "").ToString(),
				dt.Compute("sum(ModifyPrice)", "").ToString(),
				dt.Compute("sum(BalancePrice)", "").ToString(),
				dt.Compute("sum(PaymentPrice)", "").ToString(),
				dt.Compute("sum(InvoicePrice)", "").ToString()
			};
			DataRow dataRow = dt.NewRow();
			dataRow["ContractCode"] = "合计";
			dataRow["ContractPrice"] = array[0];
			dataRow["EndPrice"] = array[1];
			dataRow["ModifyPrice"] = array[2];
			dataRow["BalancePrice"] = array[3];
			dataRow["PaymentPrice"] = array[4];
			dataRow["InvoicePrice"] = array[5];
			dt.Rows.Add(dataRow);
		}
		dt.Columns["Num"].ColumnName = "序号";
		dt.Columns["ContractCode"].ColumnName = "合同编号";
		dt.Columns["ContractName"].ColumnName = "合同名称";
		dt.Columns["EndPrice"].ColumnName = "变更后的金额";
		...
		dt.Columns["ContractPrice"].ColumnName = "原始金额";
		dt.Columns["ModifyPrice"].ColumnName = "变更累计";
		dt.Columns["BalancePrice"].ColumnName = "结算累计";
		dt.Columns["PaymentPrice"].ColumnName = "已收累计";
		dt.Columns["InvoicePrice"].ColumnName = "发票累计";
		...
		dt.Columns["原始金额"].SetOrdinal(3);
		dt.Columns["变更后的金额"].SetOrdinal(4);
		dt.Columns["合同类型"].SetOrdinal(5);
		dt.Columns["项目"].SetOrdinal(6);
		dt.Columns["项目类型"].SetOrdinal(7);
		dt.Columns["签订人"].SetOrdinal(8);
		dt.Columns["签约时间"].SetOrdinal(9);
		dt.Columns["甲方"].SetOrdinal(10);
		dt.Columns["变更累计"].SetOrdinal(11);
		dt.Columns["结算累计"].SetOrdinal(12);
		dt.Columns["已收累计"].SetOrdinal(13);
		dt.Columns["发票累计"].SetOrdinal(14);
		dt.Columns["收款结算比率"].SetOrdinal(15);
		dt.Columns["合同结算比率"].SetOrdinal(16);
		dt.Columns["合同收款比率"].SetOrdinal(17);
		dt.Columns["已开发票收款比率"].SetOrdinal(18);
		dt.Columns["合同状态"].SetOrdinal(19);
		return dt;
	}
}
