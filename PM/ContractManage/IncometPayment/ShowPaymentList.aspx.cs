...
public partial class ContractManage_IncometPayment_ShowPaymentList : NBasePage, IRequiresSessionState
{
	...
	public void BindGv()
	{
		...
		{
			this.BindGv(this.incometContractBll.GetInfoByParam(this.hdContractID.Value, "Con_Incomet_Payment", base.UserCode, ""));
			return;
		}
		this.BindGv(this.incometContractBll.GetInfoByParam(this.hdContractID.Value, "Con_Incomet_Payment", base.UserCode, " AND FlowState=1"));
	}
		...
		using (SqlConnection sqlConnection = new SqlConnection(SqlHelper.ConnectionString))
		{
			...
			try
			{
				foreach (GridViewRow gridViewRow in this.gvConract.Rows)
				{
					...
					{
						int num = this.incometPaymentBll.Delete(sqlTransaction, checkBox.ToolTip);
						if (num > 0)
						{
