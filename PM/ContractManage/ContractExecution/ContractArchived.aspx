<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ContractArchived.aspx.cs" Inherits="ContractManage_ContractExecution_ContractArchived" %>
...
<head id="Head1" runat="server"><title>合同归档列表</title><link type="text/css" rel="stylesheet" href="../../Script/jquery.tooltip/jquery.tooltip.css" />
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			setButton(jwTable, 'btnArchived', 'btnRe', 'btnRe', 'hfldPurchaseChecked');
			...
			//			var trHeight = document.getElementById('trFrame').offsetHeight;
			//			document.getElementById('framContract').style.height = (trHeight) + 'px';
		});
		...
		function setbkImg(objId, hdId, param) {
			...
			if (spIds == objId)
				...
				src = getSrc(objId);
			if (!param) {
				document.getElementById("framContract").src = src + "?Archived=1&ContractID=" +
                    document.getElementById(hdId).value + "&" + Math.random();
			}
			else {
				document.getElementById("framContract").src = src + "?Archived=1&ContractID=" +
                    document.getElementById(hdId).value + "&" + param + "&" + Math.random();
			}
		}
		//选择项目
	...
	<table border="0" cellpadding="0" cellspacing="0" style="height: 100%; width: 100%;">
		<tr>
			<td style="vertical-align: top; height: 85px;">
				<table class="queryTable" cellpadding="3px" cellspacing="0px" style="">
					<tr>
						<td class="descTd">
							合同编号
						</td>
		...
		<tr style="">
			<td class="divFooter" style="vertical-align: sub; text-align: left;">
				<asp:Button ID="btnArchived" Text="归档" disabled="disabled" OnClientClick="return confirm('您确定要归档吗?')" OnClick="btnArchived_Click" runat="server" />
				<asp:Button Style="display: none;" ID="btnRe" runat="server" />
			</td>
		</tr>
		<tr>
				...
				<table class="tab">
					<tr>
						<td style="height: 100%; vertical-align: top;">
							<div>
								<asp:GridView ID="gvConract" AutoGenerateColumns="false" CssClass="gvdata" OnRowDataBound="gvPurchaseplan_RowDataBound" DataKeyNames="ContractId" runat="server"><Columns><asp:TemplateField HeaderStyle-Width="20px"><HeaderTemplate>
