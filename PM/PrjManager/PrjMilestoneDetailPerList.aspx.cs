...
using Wuqi.Webdiyer;
public partial class PrjManager_PrjMilestoneDetailPerList : NBasePage, IRequiresSessionState
{
	...
	public void BindGv()
	{
		...
		IQueryable<PrjMilestoneDetail> queryable =
			from p in this.PrjMilestoneDetailSer.AsQueryable<PrjMilestoneDetail>()
			where p.UserName == this.name.Trim() && p.RptDate == rptTime
			select p;
		decimal d = 0m;
		decimal d2 = 0m;
		decimal d3 = 0m;
	...
	public System.Collections.Generic.List<DataTable> GetData()
	{
		...
		DataTable perAllList = this.PrjMilestoneDetailSer.GetPerAllList(this.name);
		...
		for (int i = 0; i < perAllList.Rows.Count; i++)
		{
			DataRow dataRow = dataTable.NewRow();
			dataRow["序号"] = i + 1;
			dataRow["项目编号"] = perAllList.Rows[i][1].ToString();
			dataRow["项目名称"] = perAllList.Rows[i][2].ToString();
			dataRow["上报时间"] = perAllList.Rows[i][7].ToString();
			dataRow["部门名称"] = perAllList.Rows[i][6].ToString();
			dataRow["项目负责人"] = perAllList.Rows[i][4].ToString();
			dataRow["项目储备额"] = perAllList.Rows[i][8].ToString();
			dataRow["当年预测"] = perAllList.Rows[i][9].ToString();
			dataRow["储备转换率"] = Common2.FormatRate(perAllList.Rows[i][10]);
			dataRow["明年预测"] = perAllList.Rows[i][11].ToString();
			dataRow["初步接洽"] = perAllList.Rows[i][12].ToString();
			dataRow["提供样品"] = perAllList.Rows[i][13].ToString();
			dataRow["样品质量被接纳"] = perAllList.Rows[i][14].ToString();
			dataRow["投标"] = perAllList.Rows[i][15].ToString();
			dataRow["中标"] = perAllList.Rows[i][16].ToString();
			dataRow["下订单"] = perAllList.Rows[i][17].ToString();
			dataRow["交货"] = perAllList.Rows[i][18].ToString();
			dataRow["销售确认"] = perAllList.Rows[i][19].ToString();
			dataRow["项目失败"] = perAllList.Rows[i][20].ToString();
			dataRow["工程总数"] = System.Convert.ToInt32(perAllList.Rows[i][13]) + System.Convert.ToInt32(perAllList.Rows[i][14]) + System.Convert.ToInt32(perAllList.Rows[i][15]) + System.Convert.ToInt32(perAllList.Rows[i][16]) + System.Convert.ToInt32(perAllList.Rows[i][17]) + System.Convert.ToInt32(perAllList.Rows[i][18]) + System.Convert.ToInt32(perAllList.Rows[i][19]) + System.Convert.ToInt32(perAllList.Rows[i][20]) + System.Convert.ToInt32(perAllList.Rows[i][12]);
