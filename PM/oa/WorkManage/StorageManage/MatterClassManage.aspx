<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MatterClassManage.aspx.cs" Inherits="oa_WorkManage_StorageManage_MatterClassManage" %>
...
<meta http-equiv="Pragma" content="no-cache" />
	...
	function OpenWin(op)
	{
		...
		var url= "MatterClassManageEdit.aspx?t=" + op + "&lc=" + LibraryCode;
...
<HeaderStyle HorizontalAlign="Center" CssClass="grid_head"></HeaderStyle><Columns><asp:BoundField HeaderText="序号" HtmlEncode="false" /><asp:BoundField DataField="TypeCode" HeaderText="类型编号" HtmlEncode="false" /><asp:BoundField DataField="TypeName" HeaderText="类型名称" HtmlEncode="false" /><asp:BoundField DataField="Remark" HeaderText="备注" HtmlEncode="false" /></Columns><RowStyle CssClass="grid_row"></RowStyle><PagerStyle HorizontalAlign="Center"></PagerStyle><PagerSettings FirstPageText="首页" LastPageText="尾页" NextPageText="下一页" PreviousPageText="上一页"></PagerSettings></asp:GridView>
                        <asp:SqlDataSource ID="SQLDataSource" ProviderName="System.Data.SqlClient" SelectCommand="SELECT * FROM [OA_OfficeRes_Class] order by TypeCode asc" ConnectionString='<%$ ConnectionStrings:Sql %>' runat="server"></asp:SqlDataSource>
                    </div>
                </td>
            </tr>
        </table>
