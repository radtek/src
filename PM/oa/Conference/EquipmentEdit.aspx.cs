...
using System.Web.UI.WebControls;
	public partial class EquipmentEdit : BasePage, IRequiresSessionState
	{
		...
		{
			DataTable dataTable = ConferenceManage.QueryTemplateFraeInfo("OA_Meeting_Templet_Equipment", recordId);
		...
		protected void BtnSave_Click(object sender, EventArgs e)
		{
			...
			if (this.RecordID == 0)
			{
				if (ConferenceManage.AddTemplateFraeInfo("[OA_Meeting_Templet_Equipment]", hashtable))
				{
			...
			else
			{
				...
				if (ConferenceManage.UpdTemplateFraeInfo("[OA_Meeting_Templet_Equipment]", hashtable, where))
				{
					this.JS.Text = "alert('保存成功！');window.returnValue=true;window.close();";
					return;
				}
