...
public partial class Enterprise_InstitutionList : PageBase, IRequiresSessionState
{
	...
	protected void Bind_GVInsList(string leveCode)
	{
		...
		{
			string sqlString = " select * from Institutions where isvalid=1 and status=1 and ClassCode='" + leveCode + "' order by IssueDate desc ";
		...
		{
			" select * from Institutions a inner join InstitutionClass b on a.classcode=b.levecode where isvalid=1 and status=1 and (PermissionSet like '%",
	...
	protected void BtnDelete_Click(object sender, EventArgs e)
	{
		...
		if (a == "-1")
		{
			...
			{
				this.RegisterStartupScript("alert1", "<script>alert('删除成功！');</script>");
				this.Bind_GVInsList(this.TVInsClass.SelectedValue);
			}
			else
			{
	...
	protected void BtnSearch_Click(object sender, EventArgs e)
	{
		string text = string.Concat(new string[]
		{
			...
			"%' or PermissionSet like '%",
			Enterprise_InstitutionList.dRole,
			"%' or PermissionClass=-1) and isvalid=1 and status=1 and "
		});
