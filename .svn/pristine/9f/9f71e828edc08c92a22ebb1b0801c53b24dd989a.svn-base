...
using Wuqi.Webdiyer;
public partial class ContractManage_PayoutContract_ContractLedger : NBasePage, IRequiresSessionState
{
	...
	{
		System.Collections.Generic.List<PayoutContractModel> ledger = this.payoutContract.GetLedger(base.UserCode);
		...
		{
			this.AspNetPager1.RecordCount = ledger.Count;
		}
		DataTable ledger2 = this.payoutContract.GetLedger(base.UserCode, this.AspNetPager1.CurrentPageIndex, this.AspNetPager1.PageSize);
		this.gvwContract.DataSource = ledger2;
	...
	private DataTable SearchContracts()
	{
		...
		this.contracts = this.payoutContract.SelectLedger(startDate, endDate, startMoney, endMoney, this.txtContractCode.Text.Trim(), this.txtContractName.Text.Trim(), this.txtConType.Text.Trim(), this.txtProject.Text.Trim(), base.UserCode, " AND Con_Payout_Contract.FlowState = '1' ");
