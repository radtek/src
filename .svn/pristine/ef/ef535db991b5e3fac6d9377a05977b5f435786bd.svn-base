...
public partial class StockManage_Allocation_AllocationManager : NBasePage, IRequiresSessionState
{
    ...
    protected override void OnInit(System.EventArgs e)
    {
        ...
        if (HttpContext.Current.Session["yhdm"] == null || HttpContext.Current.Session["yhdm"].ToString().Length > 10)
        {
            try
            {
                string UserID = WXAPI.getUserIdByCode(code, "1000019");// "00200002";//
	...
	protected void GVAllocationList_RowDataBound(object sender, GridViewRowEventArgs e)
	{
     ...
     //               "&BusiClass=001&BusiCode=075',820,500) style='font-size: 16px; text - decoration: none;'>",
					//dataRowView["acode"].ToString(),
					//"</a>"
                    "<a class='al' href=javascript:rowQuery('"+ dataRowView["aid"].ToString()+"') style='font-size: 16px; text - decoration: none;'>",
                    dataRowView["acode"].ToString(),
                    "</a>"
                });
