<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SelectRepairPlan.aspx.cs" Inherits="Equ_Repair_SelectRepairPlan" %>
...
<head runat="server"><title>选择维修计划</title><link type="text/css" rel="stylesheet" href="../../Script/jquery.easyui/themes/default/easyui.css" />
...
<link type="text/css" rel="stylesheet" href="../../Script/jquery.tooltip/jquery.tooltip.css" />
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			jw.tooltip();
			//单击行
			$('#gvRepairPlan tr:gt(0)').live('click', function () {
				$('#hfldIdsChecked').val(this.id);
				$('#hfldCode').val($(this).attr('code'));
				$('#hfldRepairType').val($(this).attr('repairType'));
				$('#hfldRmFlag').val($(this).attr('rmFlag'));
				$('#hfldEquipmentId').val($(this).attr('equipmentId'))
			...
			$('#btnSave').click(function () {
				...
				var repairApplyInfo = {
					...
					equId: $('#hfldEquipmentId').val(), equCode: $('#hfldEquCode').val(), equName: $('#hfldEquName').val(),
					specification: $('#hfldSpecification').val(), repairType: $('#hfldRepairType').val(),
					rmFlag: $('#hfldRmFlag').val()
				};
		...
		function getEqument(id) {
			...
			$.ajax({
				type: 'GET',
				async: false,
				url: '/Equ/Handler/GetRepairPlanInfo.ashx?' + new Date().getTime() + '&id=' + id,
					...
					<table class="tab">
						<tr>
							<td style="height: 100%; vertical-align: top;">
								<div class="pagediv">
									<asp:GridView ID="gvRepairPlan" CssClass="gvdata" AutoGenerateColumns="false" OnRowDataBound="gvRepairPlan_RowDataBound" DataKeyNames="Id,EquipmentId,Code,RepairType,RmFlag" runat="server">
<EmptyDataTemplate>
...
<Columns><asp:TemplateField HeaderText="序号" HeaderStyle-Width="25px"><ItemTemplate>
													...
													<span class="tooltip" title='' onclick="Query('')">
	...
	<asp:HiddenField ID="hfldRmFlag" runat="server" />
	<asp:HiddenField ID="hfldRmFlagName" runat="server" />
	</form>
</body>
</html>
