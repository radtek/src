...
using Wuqi.Webdiyer;
public partial class ContractManage_ContractReport_PayoutInvoice : NBasePage, IRequiresSessionState
{
	private DataTable contractCount = new DataTable();
	private PayoutInvoice payoutInvoice = new PayoutInvoice();
	...
	private void DataBindContract()
	{
		...
		string[] value = new string[]
		{
			...
			this.contractCount.Compute("SUM(InvoiceTotal)", string.Empty).ToString()
		};
		int[] index = new int[]
		{
			5,
	...
	private DataTable GetData()
	{
		...
		this.contractCount = this.payoutInvoice.Select(reportCondition, base.UserCode);
		this.ViewState["contract"] = this.contractCount;
		this.AspNetPager1.RecordCount = this.contractCount.Rows.Count;
		return this.payoutInvoice.SelectData(reportCondition, base.UserCode, this.AspNetPager1.CurrentPageIndex, this.AspNetPager1.PageSize);
	}
	private DataTable GetReportData()
	{
		...
		dataTable.Columns.Remove("ContractMoney");
		dataTable.Columns.Remove("UserCodes");
		dataTable.Columns.Remove("Date");
		dataTable.Columns.Remove("InputDate");
		...
		dataTable.Columns["InvoiceTotal"].ColumnName = "发票累计";
		dataTable.Columns["InvoiceRate"].ColumnName = "发票已收比率";
		...
		dataTable.Columns["发票累计"].SetOrdinal(5);
		dataTable.Columns["发票已收比率"].SetOrdinal(6);
		...
		if (dataTable.Rows.Count > 0)
		{
			string[] array = new string[]
			{
				...
				dataTable.Compute("SUM(发票累计)", string.Empty).ToString()
			};
			DataRow dataRow2 = dataTable.NewRow();
			dataRow2["合同编号"] = "合计";
			dataRow2["最终金额"] = array[0];
			dataRow2["发票累计"] = array[1];
