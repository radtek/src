...
using System.Web.UI.WebControls;
	public partial class ReceiveFileList : BasePage, IRequiresSessionState
	{
		...
		{
			this.dgFileList.DataSource = DocumentAction.QueryReceiveFileList(this.CorpCode);
		...
		protected void btnDel_Click(object sender, EventArgs e)
		{
			...
			if (DocumentAction.DelReceiveFileInfo(fileId))
			{
		...
		protected void btnOnHole_Click(object sender, EventArgs e)
		{
			...
			text = string.Concat(new string[]
			{
				...
				this.UserCode,
				"',getDate(),Remark,FilePath,OriginalName,UserCode,RecordDate from OA_Document_ReceiveFile where fileID = '",
				guid.ToString(),
				"'"
			});
			text = text + " Update OA_Document_ReceiveFile set IsPigeonhole='1' where fileID = '" + guid.ToString() + "'";
			if (publicDbOpClass.NonQuerySqlString(text))
			{
