<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ContractLedger.aspx.cs" Inherits="ContractManage_PayoutContract_ContractLedger" %>
...
<head runat="server"><title>支出合同台帐</title><link type="text/css" rel="stylesheet" href="../../Script/jquery.tooltip/jquery.tooltip.css" />
	...
	<script type="text/javascript">
		...
		function framSrc(src) {
			if (document.getElementById('hfldContractId').value) {
				src += "?ContractID=" + document.getElementById("hfldContractId").value + "&" + Math.random();
			}
			document.getElementById("framContract").src = src;
		}

		function framSrcs(id, src) {
			document.getElementById("framContract").src = src;
			document.getElementById(id).setAttribute("onclick", "framSrc('" + src + "')", 0);
		}
				...
				<table class="tab">
					<tr>
						<td style="height: 100%; vertical-align: top;">
							<div class="">
								<asp:GridView ID="gvwContract" CssClass="gvdata" PageSize="5" AutoGenerateColumns="false" OnRowDataBound="gvwContract_RowDataBound" DataKeyNames="ContractID,IsMainContract" runat="server">
<EmptyDataTemplate>
...
<Columns><asp:TemplateField HeaderText="序号" HeaderStyle-Width="25px"><ItemTemplate>
		...
		<tr id="trFrame">
			<td style="width: 100%; vertical-align: top; padding-top: 10px;">
				<div>
					<span id="spPayoutModify" style="margin-left: 0px;" class="xxk" onclick="framSrc('/ContractManage/PayoutModify/PayoutModify.aspx')" runat="server">
						合同变更</span> <span id="spPayoutBalance" class="xxk" onclick="framSrc('/ContractManage/PayoutBalance/PayoutBalance.aspx')" runat="server">
							合同结算 </span><span id="spPayoutPayment" class="xxk" onclick="framSrc('/ContractManage/PayoutPayment/PayoutPayment.aspx')" runat="server">
								合同付款 </span><span id="spPayoutInvoice" class="xxk" onclick="framSrc('/ContractManage/PayoutInvoice/PayoutInvoice.aspx')" runat="server">
									合同发票</span>
				</div>
				<iframe id="framContract" frameborder="0" src="/ContractManage/PayoutModify/PayoutModify.aspx?ContractID=''" width="100%" runat="server"></iframe>
			</td>
		</tr>
	</table>
