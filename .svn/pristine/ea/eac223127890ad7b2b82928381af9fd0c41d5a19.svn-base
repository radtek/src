...
using Wuqi.Webdiyer;
public partial class Equ_Report_Land_RepairPlanComplete : NBasePage, IRequiresSessionState
{
	...
	public void BindGv()
	{
		...
		this.AspNetPager1.RecordCount = this.reportSer.GetRepairPlanCompleteCount("1", startDate, endDate, equCode, equName);
		DataTable roadRepairPlanComplete = this.reportSer.GetRoadRepairPlanComplete(startDate, endDate, equCode, equName, this.AspNetPager1.PageSize, this.AspNetPager1.CurrentPageIndex);
		this.gvRepair.DataSource = roadRepairPlanComplete;
	...
	protected void btnExcel_Click(object sender, System.EventArgs e)
	{
		...
		string equCode = this.txtEquCode.Text.Trim();
		string equName = this.txtEquName.Text.Trim();
		dataTable = this.reportSer.GetRoadRepairPlanComplete(startDate, endDate, equCode, equName, 0, 0);
		dataTable = this.GetTitleByTable(dataTable);
		ExcelHelper.ExportExcel(dataTable, "设备维修保养计划完成情况表", "设备维修保养计划完成情况表", "设备维修保养计划完成情况表.xls", null, null, 0, base.Request.Browser.Browser);
	}
	public DataTable GetTitleByTable(DataTable dt)
	{
		...
		{
			dt.Columns["V_XM"].ColumnName = "验收人";
		}
