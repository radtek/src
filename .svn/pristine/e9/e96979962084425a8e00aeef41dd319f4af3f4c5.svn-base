...
<head id="Head1" runat="server"><title>出库单列表</title><link href="../../Script/jquery.tooltip/jquery.tooltip.css" rel="stylesheet" type="text/css" />
	...
	<script type="text/javascript">
		...
		function rowAdd() {
			...
			var url = "/StockManage/SmOutReserve/AddReserveByXQ.aspx?prjId=" + $('#hfldPrjId').val()+"&ifXQ=byXQ";
			toolbox_oncommand(url, "新增出库单");
		}

		function rowEdit() {
			parent.parent.desktop.AddReserve = window;
			var url = "/StockManage/SmOutReserve/AddReserveByXQ.aspx?id=" + $("#hfldPurchaseChecked").val() + '&prjId=' + $('#hfldPrjId').val() + "&ifXQ=byXQ";
