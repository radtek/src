<%@ Page Language="C#" AutoEventWireup="true" CodeFile="outlist.aspx.cs" Inherits="WeChat_contract_outlist" %>
    ...
    <form id="form1" runat="server">
                ...
                <div class="search_bar">
                    <div class="search_outer">
                        <div class="search_Title qw-search_Title">
                            <a class="onetitle ellipsis">合同名称</a>
                        </div>
                        <div class="search_inner pl75">
                            <a><i class="icon_search" onclick="search()"></i></a>
                            <a><i class="icon_del qw-icon_del" onclick="qk()"></i></a>
                            <div id="temp">
                                <input type="text" class="search_input qw-search_input" name="search" id="search" placeholder="搜索合同名称" />
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script type='text/javascript'>
            ...
            function addItems(rows, page) {//获取数据函数
                ...
                $.ajax({
                    type: "POST",   //请求方式
                    url: "../contract/outlist.aspx/getList",    //请求的url地址
                            ...
                            if (res[0].data) {
                                ...
                                for (var i = 0; i < d.length; i++) {
                                    ...
                                    listHtml += "<div class=\"settings-item\">" +
                                        "<span class=\"time\">" +
                                            "<span style=\"float: right\">签约日期：" + d[i].SignDate + "</span>" +
                                            "<span style=\"float:right;margin-top:17px;margin-right:-110px\">虚拟合同：" + temp + "</span>" +
                                        "</span>" +
                                             "<div class=\"inner-settings-item flexbox\">" +
                                                    "<div class=\"user_select_icon actived\"" +
                                                    " ><label><input type=\"checkbox\" name=\"diary\" value=\"" + d[i].ContractId + "\" onclick=\"deleteDiary()\"><span></span></label></div>" +
                                                    "<div class=\"title description_title flexItem\"  onclick=\"gotoDetail('" + d[i].ContractId + "')\">" +
                                                    "<p class=\"name\" style='color: blue;'>" + d[i].ContractName + "</p>" +
                                                    "<p class=\"description description_ellipsis\">项目：" + d[i].prjName + "</p></div>" +
                                                    "</div>" +
                                               "</div>";}
                            }
                            if (type == "load") {
            ...
            function gotoDetail(Id) {
                //$.showPreloader('加载中');
                var url =  '/ContractManage/PayoutContract/ParyoutContractQueryMobel.aspx?bc=081&bcl=001&ic=' + Id;
