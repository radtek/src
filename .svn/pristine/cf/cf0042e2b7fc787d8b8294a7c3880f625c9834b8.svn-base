                            ...
                            <ContentTemplate>
                                <asp:GridView ID="gvEmployeelist" AllowPaging="true" AutoGenerateColumns="false" CssClass="grid" DataSourceID="SqlTemplate" Width="100%" OnRowDataBound="gvEmployeelist_RowDataBound" DataKeyNames="v_yhdm" runat="server">
                                    <EmptyDataTemplate>
                                        <table class="grid" cellspacing="0" rules="all" border="1" id="gvEmployeelist" style="border-collapse: collapse;">
                                            <tr class="grid_head">
                                                ...
                                                <th scope="col" style="width: 25%">部门名称</th>
                                                <th scope="col">姓名</th>
                                                  <th scope="col">手机号</th>
                                                <th scope="col" style="width: 15%">岗位</th>
                                                <th scope="col" style="width: 10%">入司日期</th>
                                    ...
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="v_xm" HeaderText="姓名" SortExpression="v_xm" />
                                        <asp:BoundField DataField="MobilePhoneCode" HeaderText="手机号" SortExpression="MobilePhoneCode" />
                                ...
                                <asp:SqlDataSource ID="SqlTemplate" SelectCommand="SELECT [v_yhdm], [v_xm],[MobilePhoneCode], [i_bmdm],(select b.v_bmmc from pt_d_bm b where b.i_bmdm = a.i_bmdm) as bmmc, [I_DUTYID],(select b.RoleTypeName from pt_d_role b where b.RoleTypeCode = (select c.DutyCode from pt_duty c where c.i_dutyId = a.i_dutyId)) as DutyName, [OtherDepts], [OtherDutyIDs], [i_xh], [c_sfyx], [MobilePhoneCode], [RelationCorp], [EnterCorpDate], [Age], [State],(case State when 0 then '试用' when 1 then '在职' when 2 then '离职' end) as StateName FROM [PT_yhmc] a WHERE ([RelationCorp] = @RelationCorp) order by v_xm" ConnectionString='<%$ ConnectionStrings:Sql %>' runat="server">
                                    <SelectParameters>
                                        <asp:QueryStringParameter Name="RelationCorp" QueryStringField="cc" Type="String"></asp:QueryStringParameter>
                                    </SelectParameters>
                                </asp:SqlDataSource>
                            </ContentTemplate>
                            <Triggers>
                                <asp:AsyncPostBackTrigger ControlID="__Page" EventName="Load" runat="server" />
                                <asp:AsyncPostBackTrigger ControlID="btnAdd" EventName="Click" runat="server" />
                                <asp:AsyncPostBackTrigger ControlID="btnDel" EventName="Click" runat="server" />
                                <asp:AsyncPostBackTrigger ControlID="btnEdit" EventName="Click" runat="server" />
                            </Triggers>
                        </asp:UpdatePanel>
                    </div>
                </td>
