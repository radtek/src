<%@ Control Language="C#" AutoEventWireup="true" CodeFile="UCSubCompanyRation.ascx.cs" Inherits="oa_WorkManage_BlocManage_UCSubCompanyRation" %>
...
<HeaderStyle HorizontalAlign="Center" CssClass="grid_head"></HeaderStyle><Columns><asp:BoundField HeaderText="序号" HtmlEncode="false" /><asp:BoundField DataField="CorpCode" HeaderText="分子机构编号" HtmlEncode="false" /><asp:BoundField DataField="CorpName" HeaderText="分子机构名称" HtmlEncode="false" /><asp:TemplateField HeaderText="定额">
<ItemTemplate>
...
</asp:TemplateField><asp:BoundField DataField="Remark" HeaderText="备注" HtmlEncode="false" /></Columns><RowStyle CssClass="grid_row"></RowStyle><PagerStyle HorizontalAlign="Center"></PagerStyle><PagerSettings FirstPageText="首页" LastPageText="尾页" NextPageText="下一页" PreviousPageText="上一页"></PagerSettings></asp:GridView>
                        <asp:SqlDataSource ID="SQLDataSource" ProviderName="System.Data.SqlClient" SelectCommand="SELECT * FROM [v_OA_OfficeRes_RationSetAndSubCompany]" ConnectionString='<%$ ConnectionStrings:Sql %>' runat="server"></asp:SqlDataSource>
                    </div>
                </td>
            </tr>
								</table>
