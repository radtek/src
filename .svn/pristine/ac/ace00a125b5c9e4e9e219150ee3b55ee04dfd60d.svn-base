...
using Wuqi.Webdiyer;
public partial class ContractManage_ContractReport_IncometBalanceReport : NBasePage, IRequiresSessionState
{
	...
	public DataTable GetTable(string strWhere)
	{
		...
		this.contractCount = this.incometContractBll.GetReportTb(this.txtConType.Text.Trim(), this.txtPrjName.Value, this.txtContractCode.Text.Trim(), this.txtContractName.Text.Trim(), this.txtStartTime.Text.Trim(), this.txtEndTime.Text.Trim(), this.txtParty.Value.Trim(), "Balance", strWhere, this.dropPrjKindClass.SelectedValue);
		this.ViewState["contract"] = this.contractCount;
		this.AspNetPager1.RecordCount = this.contractCount.Rows.Count;
		return this.incometContractBll.GetReportTb(this.txtConType.Text.Trim(), this.txtPrjName.Value, this.txtContractCode.Text.Trim(), this.txtContractName.Text.Trim(), this.txtStartTime.Text.Trim(), this.txtEndTime.Text.Trim(), this.txtParty.Value.Trim(), "Balance", strWhere, this.dropPrjKindClass.SelectedValue, this.AspNetPager1.CurrentPageIndex, this.AspNetPager1.PageSize);
	}
	...
	{
		dt = WebUtil.UpdateDataTable(dt, "BalancePrice", "Party");
		...
		dt.Columns["结算累计"].SetOrdinal(10);
		dt.Columns["合同结算比率"].SetOrdinal(11);
		...
		if (dt.Rows.Count > 0)
		{
			...
			{
				dt.Compute("sum(合同金额)", "").ToString(),
				dt.Compute("sum(结算累计)", "").ToString()
			};
			DataRow dataRow2 = dt.NewRow();
			dataRow2["合同编号"] = "合计";
			dataRow2["合同金额"] = array[0];
			dataRow2["结算累计"] = array[1];
