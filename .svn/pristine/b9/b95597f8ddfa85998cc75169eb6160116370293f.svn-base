...
using Wuqi.Webdiyer;
public partial class ProgressManagement_Modify_Ratify : NBasePage, IRequiresSessionState
{
	...
	protected void BindPlans()
	{
		...
		if (!string.IsNullOrEmpty(this.prjId))
		{
			...
			int modifyWFCount = Progress.GetModifyWFCount(userCode, this.prjId, name, version, sing, organigerName);
			this.AspNetPager1.RecordCount = modifyWFCount;
			dataSource = Progress.GetModifyWF(userCode, this.prjId, name, version, sing, organigerName, this.AspNetPager1.CurrentPageIndex, this.AspNetPager1.PageSize);
		}
	...
	protected void gvwPlans_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			if (a == "0")
			{
				...
				text8 = "流程审核";
				e.Row.Attributes["auditURL"] = text7;
			}
			else
			{
				e.Row.Cells[3].Text = "<span style='color:#008B45;'>已审核</span>";
				text7 = "/ProgressManagement/Modify/RatifyView.aspx?ic=" + text;
				text8 = "流程审核记录";
			}
