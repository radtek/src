...
using System.Web.UI.WebControls;
	public partial class Human3 : BasePage, IRequiresSessionState
	{
		private int _iSysID;
		...
		{
			string text = this.Session["HumanCode3"].ToString();
			string text2 = this.Session["HumanName3"].ToString();
			...
			for (int i = 0; i < this.LBoxHuman.Items.Count; i++)
			{
				if (this.LBoxHuman.Items[i].Selected)
				{
					...
					if (!flag2)
					{
						...
						(session2 = this.Session)["HumanCode3"] = session2["HumanCode3"] + text + "!";
						HttpSessionState session3;
						(session3 = this.Session)["HumanName3"] = session3["HumanName3"] + text2 + ",";
		...
		private void HumanList_Reset()
		{
			...
			{
				HttpSessionState session;
				(session = this.Session)["HumanCode3"] = session["HumanCode3"] + this.LBoxSelected.Items[i].Value.ToString() + "!";
				HttpSessionState session2;
				(session2 = this.Session)["HumanName3"] = session2["HumanName3"] + this.LBoxSelected.Items[i].Text.ToString() + ",";
			}
		}
