<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Oauser.aspx.cs" Inherits="oa_Vehicle_VehicleUserControl_Oauser" %>
...
<head id="Head1" runat="server"><title></title>
	...
	<script type="text/javascript">
		...
		function save() {
			$(parent.document).find('#OaUserControl1_txtPurchaser').val($('#hildSelectVal').val());
		...
		function save_VehicleNameDoubleClick(s) {
			$("#hildSelectVal").val(s);
			save();
		}
	</script>
...
<Columns><asp:TemplateField HeaderText="序号" HeaderStyle-Width="25px" HeaderStyle-HorizontalAlign="Center"><ItemTemplate>
										...
										<a class="ge" href="javascript:void(0);" ondblclick="save_VehicleNameDoubleClick('<%# Eval("v_xm") %>')"
											onclick="getVals('<%# Eval("v_xm") %>')">
											<%# Eval("v_xm") %>
										</a>
