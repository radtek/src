...
using System.Web.UI.WebControls;
	public partial class CostTaskAlter : PageBase, IRequiresSessionState
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			Label expr_177 = this.lblProjectName;
			expr_177.Text += "(预算未锁定)";
		}
		...
		private void Init_theSchedule(Guid versionCode)
		{
			...
			this._DtSchedule.Columns.Add(new DataColumn("IsContractAlter", typeof(bool)));
			...
			for (int i = 0; i < wBSBidTaskCollection.Count; i++)
			{
				...
				try
				{
					...
					dataRow["IsContractAlter"] = wBSBidTask.IsContractAlter;
				}
				catch
				{
					dataRow["IsAlter"] = false;
					dataRow["IsContractAlter"] = false;
				}
		...
		private void tblSchedule_Create(bool firstExend)
		{
			...
			tableCell = new TableCell();
			tableRow.Cells.Add(tableCell);
			tableRow.Attributes["onclick"] = "doClick(this,'" + this.tblSchedule.ClientID + "',false);clickRow(this,'','',0,0);";
		...
		private void tblSchedule_Draw(DataTable dt)
		{
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				try
				{
					if ((bool)dataRow["IsAlter"] || (bool)dataRow["IsContractAlter"])
					{
