...
using System.Web.UI.WebControls;
	public partial class CostTypeSelect : PageBase, IRequiresSessionState
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (base.Request["pc"] == null || base.Request["sd"] == null)
			{
				this.RegisterStartupScript("", "<script>alert('参数错误!');</script>");
				return;
			}
			this.ProjectCode = new Guid(base.Request["pc"].ToString());
			this.StartDate = Convert.ToDateTime(base.Request["sd"].ToString().Replace("年", "-").Replace("月", "-1"));
		...
		private void Init_theWork(Guid projectCode)
		{
			...
			ArrayList allCBSApproveList = this.theCBS.GetAllCBSApproveList();
