...
using Wuqi.Webdiyer;
public partial class ProgressManagement_Actual_Report : NBasePage, IRequiresSessionState
{
	...
	protected void gvwPlans_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		{
			string text = this.gvwPlans.DataKeys[e.Row.RowIndex]["ProgressVersionId"].ToString();
			e.Row.Attributes["id"] = text;
			e.Row.Attributes["onclick"] = "clickTr('" + text + "')";
		}
	}
