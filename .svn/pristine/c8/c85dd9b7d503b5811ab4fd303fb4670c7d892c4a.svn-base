	...
	public partial class ShowInfomation : BasePage, IRequiresSessionState
	{
		...
		protected void GVNews_RowDataBound(object sender, GridViewRowEventArgs e)
		{
			if (e.Row.RowIndex > -1)
			{
				...
				hyperLink.Attributes["title"] = dataRowView["v_xwbt"].ToString();
				hyperLink.Attributes["class"] = "firstpage";
				hyperLink.Style["cursor"] = "hand";
				string str = "/WEB/WebSel.aspx?nid=" + dataRowView["i_xw_id"].ToString() + "&cd=99";
		...
		protected void GVDBSJ_RowDataBound(object sender, GridViewRowEventArgs e)
		{
			if (e.Row.RowIndex > -1)
			{
				...
				if (ShowInfomation.htmlInputText(hyperLink.Text).Length > 12)
				{
					hyperLink.Text = ShowInfomation.htmlInputText(hyperLink.Text).Substring(0, 12) + "...&nbsp&nbsp" + str;
				}
				else
				{
					hyperLink.Text = ShowInfomation.htmlInputText(hyperLink.Text) + "...&nbsp&nbsp" + str;
				}
				hyperLink.ToolTip = dataRowView["V_Content"].ToString();
				string str2 = "/" + dataRowView["V_DBLJ"].ToString().Replace("ModuleSet", "Epc");
				hyperLink.Attributes["onclick"] = "javascript:window.open('" + str2 + "','_blank','left=150,top=150,width=600,height=400,status=0,toolbar=0,menubar=0,location=0,scrollbars=1,resizable=1',false);";
			}
		}
