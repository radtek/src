...
using System.Web.UI.WebControls;
	public partial class CodeList : PageBase, IRequiresSessionState
	{
		...
		{
			DataTable dataTable = CodeList.CodingAct.QueryDtbCodeInfo(this.TypeID, ValidState.Valid);
			...
			this.CreateChild(dataTable, dataTable2, 0, 0, "", false, firstExend);
			this.TblCodeTree_Draw(dataTable2);
		}
		private DataTable GetDataFromCollection(CodingInfoCollection colt)
		{
			DataTable dataTable = new DataTable();
			dataTable.Columns.Add(new DataColumn("NoteID", typeof(int)));
			...
			for (int i = 0; i < count; i++)
			{
				...
				dataRow["NoteID"] = colt[i].NodeID.ToString();
		...
		{
			DataRow[] array = dtSource.Select("ParentCodeID =" + parentCodeID.ToString(), "CodeID ASC");
		...
		private void TblCodeTree_Draw(DataTable dt)
		{
			...
			tableCell = new TableCell();
			tableRow.Cells.Add(tableCell);
			if (this.hfldSignCode.Value == "ProjectProperty" || this.hfldSignCode.Value == "CorpType")
			{
				tableCell = new TableCell();
				tableRow.Cells.Add(tableCell);
				this.priv.Style.Remove(HtmlTextWriterStyle.Display);
			}
			else
			{
				this.priv.Style.Add(HtmlTextWriterStyle.Display, "none");
			}
			tableRow.Attributes["onclick"] = "doClick(this,'" + this.TblCodeTree.ClientID + "');clickCodeRow('0','0',false,false,null);";
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				{
					tableCell = new TableCell();
					string text = dataRow["privilegeNames"].ToString().Trim();
					tableCell.Text = ((text.Length > 30) ? (text.Substring(0, 30) + "...") : text);
					tableCell.HorizontalAlign = HorizontalAlign.Left;
					tableCell.Width = 400;
					tableCell.ToolTip = dataRow["privilegeNames"].ToString().Trim();
					tableRow.Cells.Add(tableCell);
				}
				tableCell = new TableCell();
				...
				{
					string text2 = dataRow["ParentCodeList"].ToString();
					tableRow.Attributes["id"] = text2.Substring(0, text2.LastIndexOf(",", text2.Length - 2) + 1);
				}
				bool flag = !(bool)dataRow["IsFixed"];
				bool flag2 = (int)dataRow["ChildNumber"] == 0 && !(bool)dataRow["IsFixed"] && !(bool)dataRow["IsDefault"];
				string text3 = dataRow["NoteID"].ToString();
		...
		protected void BtnAdd_Click(object sender, EventArgs e)
		{
			...
			this.TblCodeTree_Create(true);
			com.jwsoft.pm.entpm.PageHelper.RefreshBasicCode(this, (BasicType)Enum.Parse(typeof(BasicType), this.TypeID.ToString()));
		}
		...
		{
			int num = Convert.ToInt32(this.HdnCodeID.Value);
