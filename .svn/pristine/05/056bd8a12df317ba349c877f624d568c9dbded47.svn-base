...
//参数str搜索字符串，type为搜索类型
function searchUsers(str, tp) {
    //字符串方法indexOf
...
//搜索人员并选择完成时点击确定按钮执行函数
$("#selectCheckedOk").click(function () {
    //隐藏搜索页面
    $("#searchUsers").hide();
    //显示字母排序页面
    $("#checkUsers").show();
    //执行搜索页面选择完成事件函数
    checkedSearchedUsersOk();
    //执行选中人员数组添加函数
    changeUsersArray();
});
...
function addChangeChecked() {
    ...
    if (checkedAllUsers.length == 1) {
        allCheckedHtml = "<span class=\"c999 fz14\">已选择</span>" +
            "<span class=\"cblue fz14\">" +
		...
		allCheckedHtml = "<span class=\"c999 fz14\">请选择人员</span>";
	}
    $("#checked").html(allCheckedHtml);
    //添加选中人员和人数效果
}
//数组去重复
function uniquexxx(array) {
    var r = [];
...
//搜索页面选择完成执行函数
function checkedSearchedUsersOk() {
...
function okChecked() {
    if (checkedAllUsers.length > 0) {
        ...
        for (var i = 0; i < checkedAllUsers.length; i++) {
            ...
            if (selectType == "xgr") {
                xgrForSubmit += checkedAllUsers[i].id + ",";
                classa = "remove_icon_xgr";
            }
            if (selectType == "at") {
                //处理@人部分
                atId += checkedAllUsers[i].id + ",";
                atName += checkedAllUsers[i].name + ",";
                atUrl += "<li>" +
        ...
        if (atId.length > 0) {
            atId = atId.substring(0, atId.length - 1);
        }
        if (atName.length > 0) {
        ...
        if (selectType == "at") {
            //@人部分
            $("#atUl").html(atUrl);
            var atPersonId = "<input type=\"hidden\" name=\"atPersonId\" value='" + atId + "'><input type=\"hidden\" name=\"atPersonName\" value='" + atName + "'>";
            $("#atPersonId").html(atPersonId);
        }
    }
    $("#main").show();
    $("#usersSelect").hide();
