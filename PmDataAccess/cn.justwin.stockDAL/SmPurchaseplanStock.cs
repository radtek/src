namespace cn.justwin.stockDAL
{
    ...
    public class SmPurchaseplanStock
    {
        ...
        public int Add(SqlTransaction trans, SmPurchaseplanStockModel model)
        {
            ...
            if ((model.ArrivalDateNeed == "") || (model.ArrivalDateNeed == null))
            {
                commandParameters[6].Value = DBNull.Value;
            }
            else
            {
                commandParameters[6].Value = model.ArrivalDateNeed;
            }
            if (trans == null)
            {
        ...
        {
            string str = @"SELECT Bud_TaskResource.ResourceId,ResourceCode,ResourcePrice,ResourceQuantity,ResourceQuantity*ResourcePrice ResourceSum FROM Bud_TaskResource 
                        left join Res_Resource on Res_Resource.ResourceId=Bud_TaskResource.ResourceId 
                        where PrjGuid='" + prjId + "' and ResourceCode='" + resourceCode + "'";
            DataTable dt = SqlHelper.ExecuteQuery(CommandType.Text, str);
            return dt;
        }
